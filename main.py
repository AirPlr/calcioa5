...
def main():
    ...
    if not os.path.exists(nome_file_input):
        print(f"File di input non trovato. Generazione del file template: {nome_file_input}")
        genera_template(nome_file_input)
        print(f"File template generato: {nome_file_input}. Modifica il file con i nomi dei giocatori e le loro abilità.")
        return

    giocatori = leggi_giocatori_da_file(nome_file_input)
    if len(giocatori) % 10 != 0:
        print("Il numero di giocatori non è divisibile per 10. Aggiungi o rimuovi giocatori per avere un numero di squadre pari.")
        return
